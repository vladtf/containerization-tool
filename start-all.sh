...
tmux send-keys -t monitoring-session "./start-containers-manager.sh" C-m

# Split the window horizontally, creating two vertical panes
tmux split-window -h

# Start the second script in the new pane
tmux send-keys -t monitoring-session "./start-monitoring-forwarding-rules.sh" C-m
...
tmux send-keys -t monitoring-session "./start-monitoring-traffic.sh" C-m
